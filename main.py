...
class Game(arcade.Window):
    ...
    def AI(self):
        A=0
        B=0
        if math.sqrt((self.snake.center_x-self.apple.center_x)**2+(self.snake.center_y-self.apple.center_y)**2) < math.sqrt((self.snake.center_x-self.golabi.center_x)**2+(self.snake.center_y-self.golabi.center_y)**2):
            A=self.apple.center_x
            B=self.apple.center_y
        else :
            A=self.golabi.center_x   
            B=self.golabi.center_y
        FR=True
        FL=True
        FT=True
        FB=True
